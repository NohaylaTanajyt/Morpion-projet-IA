...
int main() {
  ...
  window = SDL_CreateWindow("Morpion by Groupe 9 ", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
    600, 600, SDL_WINDOW_SHOWN);
  renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);

  setWindowColor(renderer, orange);

  drawButton(renderer, grey, 200, 200, 200, 50, "Play with AI");
  drawButton(renderer, grey, 200, 300, 200, 50, "Play with a friend");
  ...
  if (mode == 1) { // IA
    for (tour = 0; tour < tour_max && !winner(jeu, player, N); tour++) {
      player = tour % 2;

      if (player == 1) {
        do {
          waitToPlay(N, renderer, player, event, & i, & j);
          rejouer = play(jeu, i, j, 1, N);
        } while (rejouer);
    ...
    if (winner(jeu, player, N) == -1) drawButton(renderer, orange, 100, 200, 300, 50, "Egalite !");
    ...
    for (tour = 0; tour < tour_max && !winner(jeu, player, N); tour++) {
      ...
      do {
        waitToPlay(N, renderer, player, event, & i, & j);
        rejouer = play(jeu, i, j, player, N);
      } while (rejouer);
      drawGrille(N, renderer, jeu);
    }
    if (winner(jeu, player, N) == -1) drawButton(renderer, orange, 100, 200, 300, 50, "Egalite !");
    else if (winner(jeu, 0, N) == 1) drawButton(renderer, orange, 100, 200, 300, 50, "Le joueur des carres a gagne !");
